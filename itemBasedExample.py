...
with open('resources\\ejemplo.csv') as csvfile:
...
for nItem in range(item_size):
    ...
    if userCount == 0 or sumRatings1 == 0:
        neighborhood.append((nItem, 0.0))
    elif userCount < minUsers:
        neighborhood.append((nItem, (sumRatings1 / (math.sqrt(sumRatings2) * math.sqrt(sumRatings3)) ) * (userCount/minUsers)))
    else:
        neighborhood.append((nItem, sumRatings1 / (math.sqrt(sumRatings2) * math.sqrt(sumRatings3))))
...
if data[pItem][pUser] != 0:
    ...
    print("Rating for user " + str(pUser) + " and item " + str(pItem) + " is " + str(data[pItem][pUser]) + ".")
else:
    ...
    for neighbour in range(neighborhoodSize):
        nItem = neighborhood[neighbour][0]
